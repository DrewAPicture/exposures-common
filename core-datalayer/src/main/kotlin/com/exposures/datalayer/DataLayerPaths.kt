 ...
 * model: phone owns equipment/film media/photo-status, watch owns exposures.
 *
 * DataItems hold a *complete* list/map, not a per-medium slice — at this app's scale (a handful of
 * rolls/sheet packs, tens of exposures) a single combined item per entity type is well under the
 * ~100KB DataItem limit and avoids managing a dynamic set of per-medium paths.
 */
object DataLayerPaths {
    ...
    const val FILM_MEDIA = "/film-media" // phone writes, watch reads
    ...
    const val COMPLETE_FILM_MEDIUM_COMMAND = "/command/complete-film-medium" // watch -> phone (MessageClient)
    const val REQUEST_FILM_MEDIA_SYNC_COMMAND = "/command/request-film-media-sync" // watch -> phone (MessageClient)
    ...
    const val CREATE_EXPOSURE_COMMAND = "/command/create-exposure" // phone -> watch (MessageClient)
    const val CREATE_EXPOSURE_ACK_COMMAND = "/command/create-exposure-ack" // watch -> phone (MessageClient)
